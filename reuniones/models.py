...
class ZoomToken(models.Model):
    """
    La 'Billetera': Almacena las credenciales de Zoom de cada usuario.
    Relación 1 a 1: Un usuario de Django = Una cuenta de Zoom vinculada.
    """
    user = models.OneToOneField(User, on_delete=models.CASCADE, related_name='zoom_token')
    access_token = models.TextField()
    refresh_token = models.TextField()
    expires_in = models.IntegerField(help_text="Segundos para que expire")
    ...
    def is_expired(self):
        """Calcula si el token ya venció basándose en la última actualización"""
        # Margen de seguridad de 5 minutos (300 seg) antes de que expire real
        expiration_time = self.updated_at + datetime.timedelta(seconds=self.expires_in - 300)
        return timezone.now() >= expiration_time

    def __str__(self):
        return f"Token de Zoom para {self.user.username}"