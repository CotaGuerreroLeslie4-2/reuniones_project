...
class ZoomService:
    ...
    def get_user_info(self):
        """
        Obtiene la información del perfil del usuario de Zoom (email, nombre, id, etc).
        Útil para identificar al usuario y loguearlo en Django.
        """
        access_token = self.get_access_token()
        ...
        # Endpoint: /users/me devuelve los datos del dueño del token
        response = requests.get(
            f"{self.api_base_url}/users/me",
            headers=headers
        )
        ...
        if response.status_code == 200:
            ...
            raise Exception(f"Error obteniendo información del usuario: {response.text}")